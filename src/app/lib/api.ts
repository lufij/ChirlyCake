  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      try {
        ...
        throw new Error(`Error del servidor (${response.status}): ${text.slice(0, 120)}`);
      }
    }

    if (!response.ok) {
      if (response.status === 401 && data.error === 'Invalid token') {
        console.error('❌ Invalid token, clearing local token');
        setAuthToken(null);
      }
...
export async function getInventoryProducts(): Promise<{ products: InventoryProduct[] }> {
  return apiRequest('/inventory/products');
}

export async function createInventoryProduct(data: Partial<InventoryProduct>): Promise<{ product: InventoryProduct }> {
  return apiRequest('/inventory/products', { method: 'POST', body: JSON.stringify(data) });
}

export async function updateInventoryProduct(id: string, data: Partial<InventoryProduct>): Promise<{ product: InventoryProduct }> {
  return apiRequest(`/inventory/products/${id}`, { method: 'PUT', body: JSON.stringify(data) });
}

export async function deleteInventoryProduct(id: string): Promise<{ success: boolean }> {
  return apiRequest(`/inventory/products/${id}`, { method: 'DELETE' });
}

export async function createInventoryMovement(data: { productId: string; type: 'entrada' | 'salida'; quantity: number; note?: string }): Promise<{ movement: InventoryMovement; newStock: number }> {
  return apiRequest('/inventory/movements', { method: 'POST', body: JSON.stringify(data) });
}

export async function seedInventory(): Promise<{ products: InventoryProduct[] }> {
  return apiRequest('/inventory/seed', { method: 'POST' });
}

export async function getInventoryMovements(productId?: string, limit?: number): Promise<{ movements: InventoryMovement[] }> {
  const params = new URLSearchParams();
