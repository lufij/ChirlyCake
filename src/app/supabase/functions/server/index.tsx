...
const DEFAULT_PRODUCTS = [
  { category: 'Cajas para cupcakes', name: 'Caja cupcakes x4' },
  { category: 'Cajas para cupcakes', name: 'Caja cupcakes x6' },
  { category: 'Cajas para cupcakes', name: 'Caja cupcakes x12' },
  { category: 'Bases doradas', name: 'Base dorada #8' },
  { category: 'Bases doradas', name: 'Base dorada #10' },
  { category: 'Bases doradas', name: 'Base dorada #12' },
  { category: 'Bases doradas', name: 'Base dorada #14' },
  { category: 'Bases doradas', name: 'Base dorada #16' },
  { category: 'Bases doradas', name: 'Base dorada #18' },
  { category: 'Bases duroport', name: 'Base duroport #8' },
  { category: 'Bases duroport', name: 'Base duroport #10' },
  { category: 'Bases duroport', name: 'Base duroport #12' },
  { category: 'Cajas blancas con ventana', name: 'Caja blanca ventana #8' },
  { category: 'Cajas blancas con ventana', name: 'Caja blanca ventana #10' },
  { category: 'Cajas blancas con ventana', name: 'Caja blanca ventana #12' },
  { category: 'Cajas blancas con ventana', name: 'Caja blanca ventana #14' },
  { category: 'Cajas Altas', name: 'Caja alta #8' },
  { category: 'Cajas Altas', name: 'Caja alta #10' },
  { category: 'Cajas Altas', name: 'Caja alta #12' },
  { category: 'Cajas Altas', name: 'Caja alta #14' },
  { category: 'Cajas Rojas', name: 'Caja roja #8' },
  { category: 'Cajas Rojas', name: 'Caja roja #10' },
  { category: 'Cajas Rojas', name: 'Caja roja #12' },
  { category: 'Otros', name: 'Caja media plancha' },
  { category: 'Otros', name: 'Caja plancha completa' },
  { category: 'Otros', name: 'Vaso para Frappe' },
  { category: 'Otros', name: 'Bolsas de café' },
  { category: 'Otros', name: 'Caja de leche líquida' },
  { category: 'Otros', name: 'Leche condensada' },
  { category: 'Otros', name: 'Leche evaporada' },
  { category: 'Otros', name: 'Cajas para desayunos' },
  { category: 'Otros', name: 'Crema fría' },
];

// Seed idempotente: solo agrega productos que no existen por nombre
async function seedInventoryProducts() {
  const existing = await kv.getByPrefix('inventory:product:');
  ...
  for (const p of DEFAULT_PRODUCTS) {
    if (!existingNames.has(p.name)) {
      ...
      await kv.set(`inventory:product:${id}`, {
        ...
        stock: 0, minStock: 5, unit: 'unidad', createdAt: now,
      });
    }
  }
}

// GET /inventory/products  — lazy seeds on first access
app.get("/make-server-95aa99a4/inventory/products", async (c) => {
  try {
    const user = await getAuthenticatedUser(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);

    // Seed idempotente: agrega productos nuevos sin borrar los existentes
    await seedInventoryProducts();
    let products = await kv.getByPrefix('inventory:product:');
    products.sort((a: any, b: any) => a.category.localeCompare(b.category) || a.name.localeCompare(b.name));
...
// POST /inventory/seed  — force re-seed (admin/owner only)
app.post("/make-server-95aa99a4/inventory/seed", async (c) => {
  try {
    const user = await getAuthenticatedUser(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    const role = user.user_metadata?.role;
    if (!['administrador', 'propietario'].includes(role)) return c.json({ error: 'Forbidden' }, 403);
    await seedInventoryProducts();
    const products = await kv.getByPrefix('inventory:product:');
    return c.json({ products, seeded: true });
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
});
...
app.post("/make-server-95aa99a4/inventory/products", async (c) => {
  try {
    const user = await getAuthenticatedUser(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    const role = user.user_metadata?.role;
    ...
    await kv.set(`inventory:product:${id}`, product);
...
app.put("/make-server-95aa99a4/inventory/products/:id", async (c) => {
  try {
    const user = await getAuthenticatedUser(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    const role = user.user_metadata?.role;
    ...
    const existing = await kv.get(`inventory:product:${id}`);
    if (!existing) return c.json({ error: 'Product not found' }, 404);

    const body = await c.req.json();
    const updated = { ...existing, ...body, id };
    await kv.set(`inventory:product:${id}`, updated);
...
app.delete("/make-server-95aa99a4/inventory/products/:id", async (c) => {
  try {
    const user = await getAuthenticatedUser(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    const role = user.user_metadata?.role;
    ...
    const id = c.req.param('id');
    await kv.del(`inventory:product:${id}`);
    return c.json({ success: true });
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
...
// POST /inventory/movements  (entrada or salida)
app.post("/make-server-95aa99a4/inventory/movements", async (c) => {
  try {
    const user = await getAuthenticatedUser(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    const role = user.user_metadata?.role;
    ...
    // Vendedor solo puede hacer salida
    if (role === 'vendedor' && type === 'entrada') return c.json({ error: 'Sin permiso para agregar stock' }, 403);

    const product = await kv.get(`inventory:product:${productId}`);
    if (!product) return c.json({ error: 'Product not found' }, 404);

    const qty = Number(quantity);
    if (!qty || qty <= 0) return c.json({ error: 'Invalid quantity' }, 400);
    ...
    await kv.set(`inventory:product:${productId}`, { ...product, stock: newStock });
    ...
    await kv.set(`inventory:movement:${movId}`, movement);

    return c.json({ movement, newStock }, 201);
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
});

// GET /inventory/movements?productId=&limit=
app.get("/make-server-95aa99a4/inventory/movements", async (c) => {
  try {
    const user = await getAuthenticatedUser(c.req.raw);
    if (!user) return c.json({ error: 'Unauthorized' }, 401);
    ...
    let movements = await kv.getByPrefix('inventory:movement:');
    if (productId) movements = movements.filter((m: any) => m.productId === productId);
