  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {/* Hidden inputs */}
        <input type="file" accept="image/*" multiple onChange={handleImageUpload} className="hidden" id="image-upload" disabled={uploadingImage} />
        <input type="file" accept="image/*" capture="environment" onChange={handleImageUpload} className="hidden" id="camera-upload" disabled={uploadingImage} />

        <div className="flex gap-2">
          <label htmlFor="image-upload" className="flex-1 cursor-pointer">
            <div className="border-2 border-dashed rounded-lg p-4 flex flex-col items-center gap-2 text-gray-500 hover:border-pink-400 hover:text-pink-500 transition-colors">
              {uploadingImage ? <Loader2 className="w-6 h-6 animate-spin" /> : <Upload className="w-6 h-6" />}
              <p className="text-xs text-center">{uploadingImage ? 'Subiendo...' : 'Galería'}</p>
            </div>
          </label>
          <label htmlFor="camera-upload" className="flex-1 cursor-pointer">
            <div className="border-2 border-dashed rounded-lg p-4 flex flex-col items-center gap-2 text-gray-500 hover:border-pink-400 hover:text-pink-500 transition-colors">
              <Camera className="w-6 h-6" />
              <p className="text-xs text-center">Cámara</p>
            </div>
          </label>
        </div>
        {images.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-2">
            {images.map((url, idx) => (
              <div key={idx} className="relative">
                <ZoomableImage src={url} alt={`Imagen ${idx + 1}`} className="w-full h-24 object-cover rounded" />
                <button
                  type="button"
